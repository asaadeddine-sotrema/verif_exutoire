...
Ouvrez votre navigateur web (Chrome, Firefox, Edge) et accédez à l'adresse du serveur (ex: `outils.sotrema-environnement.local`).
...
### �️ Étape 4 : Sécurité et Enregistrement

Pour éviter toute pollution de la base de données par erreur, deux systèmes de sécurité ont été ajoutés :

#### 1. Confirmation d'Enregistrement (Prévention)
Lorsque vous cliquez sur **💾 Enregistrer tout en Base**, une fenêtre de confirmation s'affiche :
*   Elle vous rappelle le nombre total de lignes.
*   Elle vous alerte si le lot contient des anomalies (orphelins).
*   Vous devez valider explicitement pour pousser les données en base.

#### 2. Annulation d'Urgence (Rollback)
Si vous vous rendez compte d'une erreur juste après avoir enregistré (ex: mauvais fichier), allez immédiatement dans l'onglet **⚙️ Administration**.
*   Un bouton rouge **🚨 Annuler le dernier import** apparaîtra en haut de la page.
*   D'un seul clic, vous pouvez retirer de la base toutes les lignes du lot que vous venez d'envoyer.
*   *Note : Ce bouton n'est visible que pour l'import que vous venez de réaliser dans votre session actuelle.*

### 🗑️ Étape 5 : Archivage Manuel (Administration)

Pendant votre rapprochement des tonnages, vous pouvez supprimer les lignes qui ne sont pas pertinentes dans la section 'Administration'. Il suffit de reporter le numéro de ticket au dessus du tableau, de repérer le numéro d'identification de la ligne. Ensuite reporter là dans 'Action sur ID : 🗑️ ARCHIVER', saisissez un motif de suppression et ensuite, 'ARCHIVER LA LIGNE'. Vous pourrez toujours récupérer cette ligne en cliquant sur la case 'Voir les lignes archivées 🗑️' dans le haut de la page.
...
*   **Annulation d'Urgence** : Annuler le lot de pesées que vous venez d'importer.
*   **Archive/Restauration** : Gérer les lignes de pesées une par une (Archiver/Désarchiver).
